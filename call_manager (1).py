...
Supports: Twilio (PSTN calls), pytgcalls v3 dev (Telegram VC forwarding)
"""
...
# ─── pytgcalls v3 dev global clients ─────────────────
_pyrogram_client = None
_pytgcalls_client = None


async def get_pytgcalls_client():
    """Lazily initialize pyrogram + pytgcalls v3 client."""
    global _pyrogram_client, _pytgcalls_client

    if _pytgcalls_client is not None:
        return _pytgcalls_client

    if not (USERBOT_API_ID and USERBOT_API_HASH and USERBOT_SESSION):
        logger.warning("⚠️ Userbot credentials not set — VC forward disabled")
        return None
    ...
    try:
        ...
        _pytgcalls_client = PyTgCalls(_pyrogram_client)

        await _pyrogram_client.start()
        await _pytgcalls_client.start()

        logger.info("✅ pytgcalls v3 client started")
        return _pytgcalls_client

    except ImportError as e:
        logger.error(f"❌ Import error: {e}. Run: pip install pytgcalls==3.0.0.dev24 pyrogram tgcrypto")
        return None
...
class CallManager:
    ...
    async def forward_to_voice_chat(self, call_id: str, chat_id: int) -> bool:
        """Forward call audio to Telegram Voice Chat using pytgcalls v3 dev."""
        session = self._calls.get(call_id)
        if not session:
            return False
        ...
        try:
            ...
            client = await get_pytgcalls_client()
            if not client:
                logger.warning("⚠️ pytgcalls client not available — VC forward simulated")
                session.vc_forward = True
                return True

            # Audio source: live stream from Twilio RTP, or fallback test file
            audio_source = (
                session.recording_file
                if session.recording_file and os.path.exists(session.recording_file)
                else "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"
            )

            # pytgcalls v3 dev24 — join_group_call with MediaStream
            ...
            session.vc_forward = True
            logger.info(f"📡 VC forward active: {call_id[:8]} → chat {chat_id}")
            return True
    ...
    async def stop_vc_forward(self, call_id: str, chat_id: int) -> bool:
        """Stop VC forwarding."""
        session = self._calls.get(call_id)
        if not session:
            return False
